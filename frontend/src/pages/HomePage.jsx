...
const HomePage = () => {
  ...
  return (
    <Layout title="Discover Documents" subtitle="Browse all shared documents and explore similar content instantly.">
      ...
      <section className="rounded-3xl border border-slate-200 bg-gradient-to-br from-white via-slate-50 to-indigo-50/40 p-6 shadow-soft dark:border-slate-800 dark:from-slate-900 dark:via-slate-900 dark:to-indigo-950/30">
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {documents.map((document) => (
            <Link
              key={document.id}
              to={`/documents/${document.id}`}
              className="group rounded-2xl border border-slate-200/80 bg-white/80 p-5 transition duration-200 hover:-translate-y-1 hover:border-indigo-300 hover:shadow-soft dark:border-slate-700 dark:bg-slate-900/70 dark:hover:border-indigo-700"
            >
              ...
              <h2 className="mt-3 text-lg font-semibold text-slate-900 transition group-hover:text-indigo-700 dark:text-slate-100 dark:group-hover:text-indigo-300">{document.title}</h2>
              <p className="mt-2 line-clamp-3 text-sm text-slate-600 dark:text-slate-300">{document.description}</p>
              <p className="mt-4 text-xs font-medium text-slate-500 dark:text-slate-400">By {document.author_name}</p>
            </Link>
          ))}
        </div>
